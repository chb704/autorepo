...
handling, data exposure, dependency/configuration risk, unsafe defaults,
secret handling, and boundaries around external systems.
...
Run a static performance audit of the project codebase in this repository.
Focus on application, package, service, infrastructure, and configuration code
that affects real runtime behavior: hot paths, startup cost, build/runtime
waste, queries, caching, bundle size, expensive loops, repeated network calls,
and avoidable serialization.
